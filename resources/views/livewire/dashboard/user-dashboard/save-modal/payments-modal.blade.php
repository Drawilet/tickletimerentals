@isset($rent['id'])
    ...
    <div
        ...
        <h4 class="text-lg text-center">{{ __('calendar.total') }}: ${{ number_format($this->rent['total'], 2) }} </h4>
        ...
        <ul class="menu">
            ...
            @if ($remaining > 0)
                <form class="mt-2">
                    ...
                    <x-form-control>
                        <x-label for="payment.notes" value="{{ __('calendar.notes') }}" />
                    ...
                    <button class="btn btn-primary w-full mt-2" wire:click.prevent='addRentPayment()'>
                        <x-icons.plus />{{ __('calendar.add-payment') }}
                    </button>
                </form>
            @endif
