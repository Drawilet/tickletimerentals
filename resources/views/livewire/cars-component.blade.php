<main class="py-2 px-5 md:px-20 ">
    <div class="flex justify-center items-center w-full mb-1 flex-wrap gap-1">
        <input type="text" class="input input-bordered w-full md:w-1/2 " placeholder="{{ __('cars.Search') }}..."
            wire:model="filters.name">

        <div class="w-full mt-2 flex gap-1 md:w-1/3 md:mt-0">
            <input type="text" class="input input-bordered w-1/2" placeholder="{{ __('cars.City') }}"
                wire:model="filters.city">
            <input type="text" class="input input-bordered w-1/2" placeholder="{{ __('cars.Country') }}"
    ...
    <section class="flex gap-5 flex-wrap mt-5">
            ...
            @foreach ($filteredSpaces as $car)
                <div class="card w-96 bg-base-100 shadow-xl mx-auto md:mx-0">
                    ...
                    <div class="card-body">
                        ...
                        <p>{{ $car->description }}</p>
                        <p class="text-sm text-gray-500">{{ $car->getAddress() }}</p>
                        <div class="card-actions justify-end">
                            <button class="btn btn-primary"
                                wire:click="Modal('contact', true, '{{ $car->id }}')">{{ __('cars.Booknow') }}</button>
                        </div>
                    </div>
                </div>
            @endforeach
        @else
            <div class="w-96 mx-auto">
                <h2 class="font-bold text-xl text-center">{{ __('cars.Nocarsfound') }}</h2>
                <p class="text-center">{{ __('cars.Tryanothersearch') }}</p>
            </div>
        @endif
    </section>
        ...
        @if ($currentSpace)
            <div class="modal-box">
                ...
                <div class="modal-action">
                    <label class="btn" wire:click="Modal('contact',false)">{{ __('cars.Close') }}</label>
                </div>
            </div>
        @endif
